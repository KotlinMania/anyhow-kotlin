# Claude Code Project Instructions — anyhow-kotlin

## Project Overview

This is **anyhow-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`anyhow`](https://crates.io/crates/anyhow).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/anyhow/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/anyhow>
...
Targets: macOS arm64, Linux x64/arm64, mingw-x64, iOS arm64/x64/simulator-arm64, tvOS,
watchOS, Android Native, JS, Wasm-JS, Wasm-WASI, Android, and JVM.
