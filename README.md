...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`anyhow`](https://crates.io/crates/anyhow).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `anyhow` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Trait-object error type for application code
...
Original work copyrighted by the upstream `anyhow` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
